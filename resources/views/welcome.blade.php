        ...
        <tbody class="divide-y divide-gray-800">
            ...
            <tr>
                <td class="p-4 text-gray-400">
                    ...
                    <span class="block text-[10px] text-gray-600">{{ $latest->created_at->format('H:i:s') }}</span>
                </td>
                <td class="p-4 text-center font-mono">{{ number_format($latest->ph_air, 1) }}</td>
                <td class="p-4 text-center font-mono">{{ number_format($latest->ph_tanah, 1) }}</td>
                <td class="p-4 text-center text-orange-400">{{ $latest->suhu_udara }}°C</td>
                <td class="p-4 text-center text-blue-400">{{ $latest->kelembaban_tanah }}%</td>
            </tr>
            @else
            <tr>
...
<script>
// Chart setup untuk 1 data terbaru
...
const tempChart = new Chart(document.getElementById('tempChart'), {
    type:'bar',
    data:{labels:['Suhu Udara'], datasets:[{label:'Suhu', data:[{{ $latest->suhu_udara ?? 0 }}], backgroundColor:'#fb923c'}]},
    options:{responsive:true, scales:{y:{beginAtZero:true}}}
});

const soilChart = new Chart(document.getElementById('soilChart'), {
    type:'bar',
    data:{labels:['Kelembaban Tanah'], datasets:[{label:'Kelembaban', data:[{{ $latest->kelembaban_tanah ?? 0 }}], backgroundColor:'#38bdf8'}]},
    options:{responsive:true, scales:{y:{beginAtZero:true}}}
});
...
async function fetchLatest() {
    try {
        ...
        // Update history table, hapus sebelumnya
        const tbody = document.querySelector('#sensorTable tbody');
        tbody.innerHTML = `<tr>
            <td class="p-4 text-gray-400">Baru
                <span class="block text-[10px] text-gray-600">${new Date().toLocaleTimeString()}</span>
            </td>
